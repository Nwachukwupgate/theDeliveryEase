...
import { useAssignRiderMutation, useGetAvailableRidersQuery } from "@/api/apiSlice";
...
const AssignRiderModal: React.FC<ModalPageProps> = ({ isOpen, onClose, deliveryId }) => {
  const {
    handleSubmit,
    register,
    formState: { errors },
  ...
  const { data: ridersData } = useGetAvailableRidersQuery(
    {
      available: true,
      per_page: 100
    },
  ...
  // Submit handler
  const onSubmit = async (formData: AssignReq) => {
    console.log("delivery id", deliveryId)
    try {
      if (!deliveryId) {
      ...
      const errorMessage = typedError?.data?.message || "Assign rider failed. Please try again.";
  ...
  const riderOptions =
    ridersData?.data?.items.map((rider: Rider) => ({
